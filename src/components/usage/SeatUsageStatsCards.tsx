...
interface SeatUsageStatsResponse {
...
interface SeatUsageStatsCardsProps {
  month: number;
  year: number;
}
...
export default function SeatUsageStatsCards({ month, year }: SeatUsageStatsCardsProps) {
  const { data, loading } = useAsyncFetch<SeatUsageStatsResponse>(
    `/api/usage/seats/stats?month=${month}&year=${year}`,
  );
