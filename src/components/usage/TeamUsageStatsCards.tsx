...
interface TeamUsageStatsResponse {
...
interface TeamUsageStatsCardsProps {
  month: number;
  year: number;
}
...
export default function TeamUsageStatsCards({ month, year }: TeamUsageStatsCardsProps) {
  const { data, loading } = useAsyncFetch<TeamUsageStatsResponse>(
    `/api/usage/teams/stats?month=${month}&year=${year}`,
  );
