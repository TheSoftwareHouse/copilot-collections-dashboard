 ...
 */
async function seedAzureSession(page: import("@playwright/test").Page) {
  const client = await getClient();
  ...
  const res = await client.query(
    `INSERT INTO app_user ("username", "passwordHash") VALUES ($1, $2) RETURNING id`,
    ["azureadmin", "AZURE_AD_USER"]
  );
